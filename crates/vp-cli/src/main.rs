...
fn main() {
    ...
    match cli.command {
        ...
        Some(Command::Validate {
            ...
            if let Err(code) = run_validate(&spec, OutputOptions { format, quiet }) {
                process::exit(code);
            }
        }
    }
...
fn run_validate(spec: &PathBuf, options: OutputOptions) -> Result<(), i32> {
    if !spec.is_dir() {
    ...
    if let Err(error) = write_validation_output(&result, options, &mut stdout) {
