...
fn discover_terminology(source_file: &Path, content: &str) -> Vec<Reference> {
...
fn discover_rfc(source_file: &Path, content: &str) -> Vec<Reference> {
    static PATTERN: std::sync::OnceLock<Regex> = std::sync::OnceLock::new();
...
fn discover_architecture_sections(source_file: &Path, content: &str) -> Vec<Reference> {
    static PATTERN: std::sync::OnceLock<Regex> = std::sync::OnceLock::new();
...
fn discover_markdown_links(source_file: &Path, content: &str) -> Vec<Reference> {
    static PATTERN: std::sync::OnceLock<Regex> = std::sync::OnceLock::new();
...
mod tests {
    ...
    fn discover(content: &str) -> Vec<Reference> {
    ...
    fn discovers_architecture_section_ids() {
        ...
        assert!(refs
            .iter()
            .all(|r| r.kind == ReferenceKind::ArchitectureSection));
